...
Page({
  ...
  */
  onLoadFun:function(){
    ...
    that.getConfirm();
    //调用子页面方法授权后执行获取地址列表
    that.selectComponent('#address-window').getAddressList();
    wx.getLocation({
      type:'gcj02',
      success: function(res) {
        that.lng = res.latitude //纬度，范围为 -90~90，负数表示南纬
        that.lat = res.longitude //经度，范围为 -180~180，负数表示西经
  ...
  */
  ...
  SubOrder:function(e){
    ...
    //   this.pid_id = app.globalData.spid
    // }
